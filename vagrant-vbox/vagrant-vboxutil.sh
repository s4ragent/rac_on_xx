...
#$1 nodename $2 disksize $3 memory $4 internal_ip $5 ssh_port
	add_vagrantfile storage $VBOX_STORAGE_DISKSIZE $VBOX_STORAGE_MEMORY `get_Internal_IP storage` `common_get_ssh_port 0`

	for i in `seq 1 $nodecount`;
	do
		NODENAME="$NODEPREFIX"`printf "%.3d" $i`
		
	add_vagrantfile $NODENAME $VBOX_STORAGE_DISKSIZE $VBOX_NODE_MEMORY `get_Internal_IP $i` `common_get_ssh_port $i`
...
#$1 nodename $2 disksize $3 memory $4 intenalip $5 sshport
add_vagrantfile(){
