...
runonly(){
 ...
 docker-machine create --driver generic --generic-ip-address=`bash vagrant-vboxutil.sh get_External_IP storage` --generic-ssh-key $ansible_ssh_private_key_file --generic-ssh-user $ansible_ssh_user storage
 
	for i in `seq 1 $nodecount`;
	do
		NODENAME="$NODEPREFIX"`printf "%.3d" $i`
		docker-machine create --driver generic --generic-ip-address=`bash vagrant-vboxutil.sh get_External_IP $i` --generic-ssh-key $ansible_ssh_private_key_file --generic-ssh-user $ansible_ssh_user $NODENAME
	done
