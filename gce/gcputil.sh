...
create_ubuntu(){
		name=$1
		gcloud compute instances create $name --machine-type $2 --network "default" --can-ip-forward --maintenance-policy "MIGRATE" --scopes "https://www.googleapis.com/auth/devstorage.read_write,https://www.googleapis.com/auth/logging.write" --image ubuntu-1604-lts --boot-disk-type "pd-ssd" --boot-disk-device-name $name --boot-disk-size $3
}
